        ...
        for d in self.data:
            ...
            if item.name == input_name:
                for a in range(len(item.availability)):
                    if item.availability[a].color == input_color:
                        if item.availability[a].size == input_size:
...
shop1.select_item_as_dataclass("Dress", "pink", "S")
